...
class User
{
    ...
    private static function getUser($identification)
    {
        ...
        if ($identification && self::$uid != $identification) {
            ...
            $user = R::load(TABLE_USER, $identification);
            if (!$user->id) {
                return new EmptyUser($identification);
            }
        ...
        elseif (self::isAuth() && !self::$user) {
            self::$isCurrentAuth = true;
            $user = R::load(TABLE_USER, self::$uid);
            $userCache =& self::$user;
        ...
        if (!file_exists($newUserClass_path)) {
            throw new AppException (sprintf("Файла '%s' для типа пользователя '%s' не найден", $newUserClass_path,
                $newUserClass));
        }
